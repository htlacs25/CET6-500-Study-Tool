...
import { buildRecoveryLesson, blankProgress, recordFirstAttempt, mergeLegacyStores, assessSentence, assessTranslation, readiness, courseDay, phaseFor, chooseDailyLesson, hasAnswerWork, RECOVERY_STORAGE_KEY, OLD_PROGRESS_KEYS } from '../lib/study-engine.mjs';
...
test('a seven-day window has different actual reading/listening passages and matching local copies',async()=>{
  ...
  const beyond=await prepareStandalone('2026-09-11');assert.ok(beyond.release.missingDates.length>0);
});

test('only unfinished preview caches refresh; existing answers and even mismatched snapshots are not silently reassigned',()=>{
  const fresh=buildRecoveryLesson(DATA,RECOVERY,'2026-08-30'),old={...fresh,title:'旧预览',contentRevision:'old'};
  ...
  p.draft.ws={input:'time'};assert.equal(chooseDailyLesson('2026-08-30',p,fresh).title,'旧预览');
  const mismatch=chooseDailyLesson('2026-08-31',p,fresh);assert.equal(mismatch.cacheDateMismatch,true);
  assert.equal(mismatch.title,'旧预览');assert.equal(p.notes,'必须保留的难点');
});

test('many unresolved old errors use at most four rotating priority slots',()=>{
  const errors=['organize','information','difficulty','discipline','diverse','domestic','assess'];
  const history={'2026-08-25':{...blankProgress(),wordErrors:errors}};
  const a=buildRecoveryLesson(DATA,RECOVERY,'2026-08-30',history),b=buildRecoveryLesson(DATA,RECOVERY,'2026-08-31',history);
...
test('wrong and overdue words get fixed review slots; later first-try success resolves error priority',()=>{
  ...
  assert.deepEqual(next.rev.slice(0,2).map(x=>x.word),['improve','enough']);
  assert.equal(next.rev.length,12);assert.equal(next.nw.length,8);
  const later=buildRecoveryLesson(DATA,RECOVERY,'2026-09-02',history);
  assert.equal(later.rev[0].word,'improve');
  ...
  assert.equal(buildRecoveryLesson(DATA,RECOVERY,'2026-08-30',history).rev[0].word,'enough');
});
...
test('readiness uses sufficient completed data, not diagnostic guesses or future work',()=>{
  ...
  assert.equal(buildRecoveryLesson(DATA,RECOVERY,'2026-09-11',h).foundation,false);
...
function runtime(initial={},browserMode=false){
  ...
  const window={location:{href:'file:///D:/CET6-500-Study-Tool/六级学习工具.html',reload:()=>reloadCount++},addEventListener:(name,fn)=>events[name]=fn};
  ...
  code=code.replace(marker,'globalThis.studyTest={lesson,progress,record,save,reset,render,renderWords,renderListening,renderPractice,renderRecords,persistDraft,speak,seven,state:()=>({ws,ls,ps,rate,selected,today}),setDate:k=>{selected=k;reset()},setTab:k=>{tab=k}};reset();render();setInterval');
...
test('a local release update waits for typing to stop, preserves drafts and first scores, and avoids reload loops',()=>{
  ...
  rt.window.__CET6ReceiveRelease({buildId:'next-local-version'});assert.equal(rt.reloads(),1);
});
